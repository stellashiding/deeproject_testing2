...
export function renderScenario(root) {
  ...
  root.innerHTML = `<div class="page scenario-page">
    ...
    <div class="scenario-layout">
      <details class="context-panel context-disclosure"><summary><span class="context-view-label">View context</span><span class="context-hide-label">Hide context</span></summary><div class="context-disclosure-body"><h2>Key constraints</h2>${item.constraints.map(c => `<div class="constraint">✓ ${esc(c)}</div>`).join("")}${item.retrieval ? `<h3>Retrieved context</h3>${item.retrieval.map(r => `<div class="retrieval-item">${esc(r)}</div>`).join("")}` : ""}<div class="notice">Rate the AI response. Select any message as supporting evidence.</div></div></details>
